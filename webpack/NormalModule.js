...
class NormalModule extends Module {
	...
	source(dependencyTemplates, runtimeTemplate, type = "javascript", entryId) {
		const cacheKey = `${type}.${entryId}`;
		const hashDigest = this.getHashDigest(dependencyTemplates);
		const cacheEntry = this._cachedSources.get(cacheKey);
		...
		this._cachedSources.set(cacheKey, {
			source: cachedSource,
			hash: hashDigest
		});
		return cachedSource;
